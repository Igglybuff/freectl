...
export function loadSourceFilter() {
  fetch("/sources/list")
    ...
    .catch((error) => {
      ...
      const errorOption =
        '<option value="">Error loading data sources</option>';
  ...
  fetch("/sources/list")
    ...
    .then((data) => {
      ...
      data.sources.forEach((source) => {
        ...
        if (
          source.last_updated &&
          source.last_updated !== "0001-01-01T00:00:00Z"
        ) {
  ...
  fetch("/sources/delete", {
    ...
    .catch((error) => {
      ...
      if (sourceItem && sourceItem.dataset.originalButtons) {
        sourceItem.querySelector(".source-buttons").innerHTML =
          sourceItem.dataset.originalButtons;
        delete sourceItem.dataset.originalButtons;
      }
    });
}
...
export function processSource() {
  const processButton = document.getElementById("processSource");
  processButton.disabled = true;
  processButton.textContent = "Processing sources...";
  ...
  fetch("/process", {
    ...
    .catch((error) => {
      console.error("Error:", error);
      showToast("Failed to process sources", "error");
    })
    .finally(() => {
      processButton.disabled = false;
      processButton.textContent = "Process sources";
    });
}
