...
document.addEventListener("DOMContentLoaded", function () {
  ...
  loadSettings().then(() => {
    ...
    document
      .getElementById("favoriteSourceFilter")
      .addEventListener("change", function () {
        updateFavoritesDisplay();
      });
    ...
    searchInput.addEventListener("input", function () {
      ...
      if (validateSearchInput(this)) {
        searchTimeout = setTimeout(
          () => performSearch(1),
          getCurrentSettings() ? getCurrentSettings().searchDelay : 300,
        );
      }
    });
    ...
    document
      .getElementById("favoritesSearchInput")
    ...
    // Add event listener for preprocessed search checkbox
    document
      .getElementById("usePreprocessedSearch")
      .addEventListener("change", function () {
        const isEnabled = this.checked;
        showToast(
          `${isEnabled ? "Enabled" : "Disabled"} preprocessed search`,
          "info",
        );

        // Save settings immediately
        saveSettings().catch((error) => {
          console.error("Failed to save preprocessed search setting:", error);
    ...
    document
      .getElementById("favoriteCategoryFilter")
      .addEventListener("change", function () {
        updateFavoritesDisplay();
      });
    ...
    document
      .getElementById("updateSource")
      .addEventListener("click", updateSource);

    // Add event listener for process sources button
    document
      .getElementById("processSource")
      .addEventListener("click", processSource);
  });
});
