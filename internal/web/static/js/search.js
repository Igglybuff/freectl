...
export function updateCategoryFilter(results, selectedCategory = "") {
  ...
  Array.from(allSearchCategories)
    .sort()
    .forEach((category) => {
...
function createResultHTML(result, showScore = true) {
  const isFavorite = document
    .querySelector(`.favorite-btn[data-link="${result.url}"]`)
  ...
  if (showScore && currentSettings && currentSettings.showScores) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      bars.push(
        `<div class="score-bar ${isActive ? "active" : ""} ${colorClass}"></div>`,
      );
    }
...
function addDescriptionToggleListeners() {
  document.querySelectorAll(".result-description-toggle").forEach((button) => {
    button.addEventListener("click", function () {
      const description = this.closest(".result-content").querySelector(
        ".result-description",
      );
...
export function performSearch(page = 1) {
  ...
  const resultsPerPage = getCurrentSettings()
    ? getCurrentSettings().resultsPerPage
    : 10;
    ...
    .then((data) => {
      if (!data || !data.results || data.results.length === 0) {
        resultsDiv.innerHTML =
          '<div class="no-results">No results found. Go to Settings to update your sources.</div>';
        return;
      }
